 ...
 * @return {?Object}
 */
function getMockInfoByQueryParamRules(method, queryInfo, queryRules) {
    if (!queryRules) {
        return null;
    }
...
exports.findMatchMockRule = function (method, pathname, options, reqURL) {
    ...
    var foundMock;
    for (var i = 0, len = mockRules.length; i < len; i++) {
        foundMock = exports.match(method, pathname, mockRules);
        if (foundMock) {
            break;
        }
    }
    ...
    if (!foundMock) {
        foundMock = getMockInfoByQueryParamRules(
            method, reqURL.query || {}, options.query
        );
    }
