 ...
 * @param {string=} callbackKey jsonp 回调参数名
 * @return {{data: string, type: string}}
 */
function getJSONPResponseData(data, context, callbackKey) {
    var query = context.url.query;
    callbackKey || (callbackKey = context.callbackKey || 'callback');
    var cbFuncName = query[callbackKey];
    ...
    if (!cbFuncName) {
        logger.error(
            'response jsonp missing `%s` query param: %s',
            callbackKey, context.url.path
        );
    }
        ...
        function (err, result, dataType) {
            if (err) {
                logger.error(
                    ...
                    JSON.stringify(data), mockFile.path, err.stack || err
                );
                autoresponseUtil.response500(context.res);
            }
            else {
                ...
                var isJSONP = result.jsonp;
                if (isJSONP || (mockFile.jsonp && isJSONP !== false)) {
                    _.assign(
                        result,
                        getJSONPResponseData(data, context, result.callback)
                    );
                }
