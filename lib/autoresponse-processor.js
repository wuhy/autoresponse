...
function postprocessResponseData(mockFile, context, options, callback) {
    ...
    var processor = getProcessor(rawData._process, options);
    ...
    postprocessResponseData(
        ...
        function (err, result, dataType) {
            if (err) {
                ...
                if (isJSONP) {
                    result = getJSONPResponseData(data, context);
                }
                ...
                writeResponse(context.res, result, options.timeout);
            }
        }
    );
}
